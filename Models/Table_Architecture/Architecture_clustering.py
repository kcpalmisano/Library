...
# path = r"C:/Users/PATH/MedicalClaims2.0 Field ChangeLog.xlsx"

# # Load your excel data into a DataFrame
# data = pd.read_excel(path, sheet_name='ClaimsData')  
...
sql = pd.read_sql('''
                  ...
                  where YEAR(servicestartdate) >= 2022  --2022 
                  ''', con)

sql1 = pd.read_sql('''
                  select top 100000 * from TABLE
                  where YEAR(servicestartdate) >= 2023   
                  ''', con)
                  
                  
sql2 = pd.read_sql('''
                  select top 100000 * from TABLE
                  where YEAR(servicestartdate) >= 2024   
                  ''', con)



sql = pd.concat((sql, sql1, sql2))
...
df_fil = sql.drop(columns = ['COLUMN1', 'COLUMN2'], axis=1)
...
full_matrix = corfil_matrix.fillna(0)
...
##############################################
# Minimum correlation threshold (adjust as needed)
corr_threshold = 0.12
...
# # Annotate only above threshold (modify mask logic if needed)
...
f_thresh = 0.5  # Correlation threshold 
full_corr_pairs = []
...
n_clusters = elbow_method(full_matrix)  # Estimate optimal number of clusters
print("Estimated number of clusters:", n_clusters)



kmeans = KMeans(n_clusters=n_clusters, random_state=56)  # Set random state for reproducibility
...
#df.to_csv('C:/Users/PATH/correlation.csv')
...
############## Certain columns 
sub_columns = full_matrix[['ProfitabilityCode','ProviderLocationCode','ProviderContractType','BenefitPaymentTierCode',
                   'PackageNr','RateCategory','RateSubcategory' ]]


# Perform hierarchical clustering on the subset data
row_linkage = linkage(sub_columns, method='ward', metric='euclidean')
col_linkage = linkage(sub_columns.T, method='ward', metric='euclidean')
...
#df_ordered.to_csv('C:/Users/PATH/correlation1.csv')
